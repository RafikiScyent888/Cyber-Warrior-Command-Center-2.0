# Cyber Warrior Command Center 2.0

The master hub for the **Cyber Warrior Program** — central landing page for
every CompTIA A+, Network+, Security+, and CySA+ study resource built so far.
This is the successor to the original `Cyber-Warrior-Command-Center` repo,
which now redirects here.

Live site: https://rafikiscyent888.github.io/Cyber-Warrior-Command-Center-2.0/

## What's different from 1.0

Two layers now work together instead of one:

1. **This hub** — one page, all five subject areas, direct links to every
   quiz/sim/acronym site, searchable.
2. **Standalone tile pages** — each subject area also has its own single-tile
   page (Royal Blue background, one colored tile, same links) that can be
   shared on its own without the rest of the hub. Every tile here has a
   "Full Tile ↗" button linking to its standalone counterpart.

| Subject | Standalone tile |
|---|---|
| A+ Core 1 | https://rafikiscyent888.github.io/A-Core-1-tile/ |
| A+ Core 2 | https://rafikiscyent888.github.io/A-Core-2-tile/ |
| Network+ | https://rafikiscyent888.github.io/Network-Tile/ |
| Security+ | https://rafikiscyent888.github.io/Security-Tile/ |
| CySA+ | https://rafikiscyent888.github.io/CySA-Tile/ |
| Universal | https://rafikiscyent888.github.io/Universal-tile/ |
| Face Off Games | https://rafikiscyent888.github.io/Face-Off-Games/ |

## What's here

- `index.html` — the command center itself. Plain HTML/CSS/vanilla JS, no
  build step. Color-coded tiles link out to every resource site plus their
  standalone counterpart, with a search bar to filter by keyword.
  Mobile-friendly.
- `CONCEPTS.md` — cross-repo concept index: which topics are already
  covered, in which repos, in what format, and where taxonomies or content
  don't yet line up across repos.
- `CLAUDE.md` — full project context (repo inventory, formats, known content
  issues) for anyone (human or AI assistant) picking this project back up.

## Linked resources

**A+ (Core 1 & Core 2)**
- [Core 1 Quizzes](https://rafikiscyent888.github.io/Core-1-quizzes/) · [Core 1 Sims](https://rafikiscyent888.github.io/Core-1-Sims/) · [Core 1 Full Tile](https://rafikiscyent888.github.io/A-Core-1-tile/)
- [Core 2 Quizzes](https://rafikiscyent888.github.io/Core-2-quizzes/) · [Core 2 Sims](https://rafikiscyent888.github.io/Core-2-Sims/) · [Core 2 Full Tile](https://rafikiscyent888.github.io/A-Core-2-tile/)
- [A+ Acronyms](https://rafikiscyent888.github.io/A-acronym-games./) (covers both Core 1 & 2)

**Network+**
- [Network Quizzes](https://rafikiscyent888.github.io/Network-quizzes/) · [Network Acronyms](https://rafikiscyent888.github.io/Network-Acronyms/) · [Network Sims](https://rafikiscyent888.github.io/Network-Sims/) · [Full Tile](https://rafikiscyent888.github.io/Network-Tile/)

**Security+**
- [Security Quizzes](https://rafikiscyent888.github.io/Security-Questions/) · [Security Acronyms](https://rafikiscyent888.github.io/Security-Acronyms/) · [Security Sims](https://rafikiscyent888.github.io/Security-Sims/) · [Full Tile](https://rafikiscyent888.github.io/Security-Tile/)

**CySA+** 
- [CySA Quizzes](https://rafikiscyent888.github.io/CySA-questions/) · [CySA Acronyms](https://rafikiscyent888.github.io/CySA-Acronyms/) · [CySA Sims](https://rafikiscyent888.github.io/CySA-Sims/) · [Full Tile](https://rafikiscyent888.github.io/CySA-Tile/)

**Universal**
- [Port Quiz](https://rafikiscyent888.github.io/Port-Quiz/) · [Windows & Linux Commands](https://rafikiscyent888.github.io/Windows-Linux-Commands/) · [Patch Bay](https://rafikiscyent888.github.io/Patch-Bay/) · [Subnet Calculator](https://rafikiscyent888.github.io/Subnet-Calculator-for-Network/) · [OSI Model Lab](https://rafikiscyent888.github.io/OSI-Model/) · [IPv6 & EUI-64 Drills](https://rafikiscyent888.github.io/IPv6-Drills/) · [CySA CVSS Center](https://rafikiscyent888.github.io/CySA-CVSS-Center/) · [Cloud Ops: Basic Training](https://rafikiscyent888.github.io/Cloud-aaS-Learning-Hub/) · [Full Tile](https://rafikiscyent888.github.io/Universal-tile/)

**Face Off Games**
- [A+ Core 1](https://rafikiscyent888.github.io/Face-off-A-Core-1/) · [A+ Core 2](https://rafikiscyent888.github.io/Face-off-A-Core-2/) · [Network+](https://rafikiscyent888.github.io/Face-off-Networking/) · [Security+](https://rafikiscyent888.github.io/Face-off-Security/) · [CySA+](https://rafikiscyent888.github.io/Face-off-CySA/) · [Full Tile](https://rafikiscyent888.github.io/Face-Off-Games/)

**Interactive Labs**
- [A+ Core 1 Field Service Center](https://rafikiscyent888.github.io/A-Core-1-Field-Service-Center/) · [A+ Core 2 Field Service Center](https://rafikiscyent888.github.io/A-Core-2-Field-Service-Center/) · [Patch Bay](https://rafikiscyent888.github.io/Patch-Bay/) · [OSI Model Lab](https://rafikiscyent888.github.io/OSI-Model/) · [Subnet Calculator](https://rafikiscyent888.github.io/Subnet-Calculator-for-Network/) · [IPv6 & EUI-64 Drills](https://rafikiscyent888.github.io/IPv6-Drills/) · [Security Awareness Center](https://rafikiscyent888.github.io/Security-Awareness-Center/) · [CySA CVSS Center](https://rafikiscyent888.github.io/CySA-CVSS-Center/) · [Full Tile](https://rafikiscyent888.github.io/Interactive-labs/)

## Hosting

GitHub Pages, deployed from `main` / root — Settings → Pages → Source:
Deploy from a branch → `main` → `/ (root)`.

## Disclaimer

For educational purposes only. Not affiliated with, endorsed by, or
sponsored by CompTIA®. All trademarks belong to their respective owners.
